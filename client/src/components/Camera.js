...
export const Camera = () => {
    ...
    const handleStreamVideo = async (e) => {
        ...
        detection.current = setInterval(async () => {
            if (counter <= 40 && webcamRef.current && webcamRef.current.video) { // Check if webcamRef.current and webcamRef.current.video are not null
    ...
    return(
        <div className="camera-container" style={{backgroundImage: 'url("/images/facebg.jpg")'}} ref={containerRef}>
            {(cameraStatus === 'opened') &&
                <>
                <Webcam
